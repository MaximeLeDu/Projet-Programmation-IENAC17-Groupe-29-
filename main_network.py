...
def jeu(nb_parties,agent,p):
    for i_episode in range(nb_parties):
        # Initialize the environment and state
        p.reset_game()
        state = p.game.getGameState()
        state = agent.FloatTensor([state[value] for value in agent.states])
        for t in count():
            ...
            if not p.game_over():
                ...
                next_state = agent.FloatTensor([next_state[value] for value in agent.states])
            else:
                next_state = None
