...
from ple import PLE,save_files
    ...
    def training(self, reward, state):
        ...
        if self.last_action!=None:
            ...
            if choix > RANDOM_PARADIGM:
                ...
                action = np.random.randint(0, len(self.actions))
            self.rewards[self.last_state + (self.last_action,)]+=ALPHA*(reward*10e17 + GAMMA*np.max(self.rewards[state]) - self.rewards[self.last_state + (self.last_action,)])
...
save_files.uni_to_fichier(save_files.multi_to_uni_no_zeros(agent.rewards),"ple/pong_rewards.txt")
save_files.save_state("ple/pong_states.txt",agent)
